...
void sig_handler(int sig_numb) { // signal handling
    char ch;

    call_numb++;

    printf(
        "sig_handler(signal %d) - %dth call at address %p\n",
        sig_numb, call_numb, (void*)last_addr);
    puts("Exit process [y/n]");

    if((ch = getchar()) == 'y') exit(0); // terminate proc

    sbrk(512); // set 512 memory bytes
};

int main(void) {
    signal(SIGSEGV, sig_handler); // set reaction
